""" Investment Analyst AI – Regulus Edition | Original UI Restored """
...
st.markdown(
    ...
    <div style="background:linear-gradient(135deg,#1B2B4D 0,#2C3E5E 100%);
        ...
        <div style="position:absolute;top:25px;left:35px;">
            <img src="{qdb_logo}" style="max-height:70px"/>
        </div>
...
st.markdown(
    """
    <style>
    .btnrow {display:flex;flex-wrap:wrap;justify-content:center;gap:20px;margin:60px 0 40px 0;}
    .mainbtn {
    ...
    .mainbtn.teal {background-color:#319795;}
    .mainbtn.teal:hover {background-color:#2C7A7B;}
    .mainbtn.gray {background-color:#2D3748;}
    .mainbtn.gray:hover {background-color:#4A5568;}
    </style>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    """
    <div class="btnrow">
        <a href="/pages/1_Deal_Sourcing" class="mainbtn teal">Deal Sourcing</a>
        <a href="/pages/2_Due_Diligence" class="mainbtn gray">Due Diligence</a>
        <a href="/pages/3_Market_Analysis" class="mainbtn teal">Market Analysis</a>
        <a href="/pages/4_Financial_Modeling" class="mainbtn gray">Financial Modeling</a>
        <a href="/pages/5_Investment_Memo" class="mainbtn teal">Investment Memo</a>
    </div>
    """,
    unsafe_allow_html=True,
)
...
def section_card(title, theme, points, link):
    bg = "#fff" if theme == "light" else "#1B2B4D"
    txt = "#1B2B4D" if theme == "light" else "#E2E8F0"
    btn = "#319795" if theme == "light" else "#2D3748"
    hover = "#2C7A7B" if theme == "light" else "#4A5568"
    st.markdown(
        f"""
        <div style="background:{bg};color:{txt};border-radius:16px;padding:35px;
                    height:390px;box-shadow:0 5px 20px rgba(0,0,0,0.08);border-top:4px solid #2C3E5E;
            ...
            <div style="text-align:center;margin-top:10px;">
                <a href='/{link}' target='_self' style='text-decoration:none;
                   background:{btn};color:white;padding:10px 28px;
                   border-radius:30px;font-weight:600;
                   transition:background 0.3s ease;'>Start →</a>
                <style>a[href='/{link}']:hover{{background:{hover}!important;}}</style>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with col1:
    section_card(
        "Deal Sourcing Path",
        "light",
        ["AI‑guided deal identification", "Market mapping hand‑off", "Scoring & export to Due Diligence"],
...
with col2:
    section_card(
        "Due Diligence Path",
        "dark",
        ["Upload & analyze reports", "Evaluate risk dimensions", "Integrate into Investment Memo"],
