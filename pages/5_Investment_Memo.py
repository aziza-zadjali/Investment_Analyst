...
def generate_memo(data):
    """Generate investment memo from data"""
...
- **Current Annual Revenue:** {data['current_revenue']}
- **Revenue Growth Rate:** {data['revenue_growth']}

---

## FINANCIAL PERFORMANCE

- **Annual Revenue:** {data['current_revenue']}
...
"""
    ...
    if data.get('scoring'):
        s = data['scoring']
        memo += f"""## STRATEGIC ASSESSMENT FRAMEWORK

### Gulf Resonance Scoring

| Dimension | Score | Description |
|-----------|-------|-------------|
| **Strategic Clarity** | {s['strategic_clarity']}/10 | Clear vision and strategic roadmap |
| **Symbolic Fluency** | {s['symbolic_fluency']}/10 | Brand positioning and narrative |
| **Execution Discipline** | {s['execution_discipline']}/10 | Track record of delivery |
| **Archetypal Fit** | {s['archetypal_fit']}/10 | Portfolio alignment |
| **Gulf Resonance** | {s['gulf_resonance']}/10 | Regional market fit |
...
if selected_deal:
    ...
    st.info("💡 Complete previous workflow steps for auto-fill")
...
with col1:
    ...
    founded_year = st.text_input("Founded", placeholder="2020")
with col2:
    industry = st.text_input("Industry *", value=industry_default)
    location = st.text_input("Location", placeholder="Doha, Qatar")
with col3:
    stage = st.selectbox("Stage *", ["Seed", "Series A", "Series B", "Series C", "Growth"], index=0)
    deal_size = st.text_input("Investment *", placeholder="$5M")

col4, col5, col6 = st.columns(3)
with col4:
    valuation = st.text_input("Valuation *", placeholder="$50M")
with col5:
    ownership = st.text_input("Ownership %", placeholder="10%")
with col6:
...
with col_bus1:
    ...
    investment_thesis = st.text_area("Investment Thesis", height=80)
with col_bus2:
    products_services = st.text_area("Products/Services", height=80)
    key_highlights = st.text_area("Key Highlights", height=80)
...
with col_q2:
    current_revenue = st.text_input("Revenue", placeholder="$10M")
with col_q3:
    growth_rate = st.text_input("Growth", placeholder="150%")

# Scoring (Optional)
with st.expander("📊 Optional: Scoring Framework"):
    col_s1, col_s2 = st.columns(2)
    with col_s1:
        strategic_clarity = st.slider("Strategic Clarity", 1, 10, 7)
        symbolic_fluency = st.slider("Symbolic Fluency", 1, 10, 6)
        execution_discipline = st.slider("Execution", 1, 10, 6)
    with col_s2:
...
if st.session_state.memo_complete:
    ...
    with col_dl1:
        st.download_button("📥 Download Markdown", st.session_state.memo_content, f"{st.session_state.memo_data['company_name'].replace(' ', '_')}_Memo.md", "text/markdown", use_container_width=True)
    with col_dl2:
        try:
            doc = template_gen.markdown_to_docx(st.session_state.memo_content)
            bio = BytesIO()
            doc.save(bio)
            st.download_button("📥 Download DOCX", bio.getvalue(), f"{st.session_state.memo_data['company_name'].replace(' ', '_')}_Memo.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
        except:
            pass
